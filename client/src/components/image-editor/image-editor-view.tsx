          ...
          <div className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="min-h-0 h-full basis-[47%] shrink-0 border-l border-gray-200 bg-gray-100 p-2">
              <div className="flex h-full flex-col gap-2 overflow-y-auto">
                ...
                <button onClick={() => handleAutoArrange({ preserveSelection: selectedDesignIds.size >= 2 })} disabled={designs.length < 2 && selectedDesignIds.size < 2} className={`mx-auto rounded-md px-2 py-2 text-[11px] font-medium transition-all ${designs.length >= 2 || selectedDesignIds.size >= 2 ? "border border-[#CBD5E1] bg-[#F1F5F9] hover:bg-[#E2E8F0] text-[#0891B2] shadow-none" : "pointer-events-none bg-gray-200 text-gray-500 opacity-30"}`} title={selectedDesignIds.size >= 2 ? t("editor.autoArrangeSelected") : t("editor.autoArrangeAll")}><span className="inline-flex items-center justify-center gap-1"><LayoutGrid className="h-3.5 w-3.5" />{t("editor.autoArrange")}</span></button>
