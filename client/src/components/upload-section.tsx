  ...
  const handleFileUpload = useCallback(
    async (file: File) => {
      ...
      img.onerror = () => {
        URL.revokeObjectURL(originalUrl);
        toast({
          title: t("toast.failedLoad"),
          description: t("toast.failedLoadDesc"),
          variant: "destructive",
        });
      };
      img.src = originalUrl;
    ...
    <div className={isEmptyState ? "w-full" : "flex-shrink-0"}>
        ...
        style={
          isEmptyState
            ? {
                ...
                transition:
                  "background 1.5s ease-in-out, box-shadow 1.5s ease-in-out",
              }
            : undefined
        }
      >
        <div
          className={`flex items-center ${isEmptyState ? "flex-col" : "gap-1.5"}`}
        >
          {isEmptyState && (
            <>
              ...
              <p className="text-sm text-white/80 mb-4">
                {t("upload.preferredFormat")}&nbsp;:&nbsp;{" "}
                <span className="font-semibold text-white">
                  {t("upload.pngTransparent")}
                </span>
              </p>
            </>
          )}
          {!isEmptyState && <Upload className="w-3.5 h-3.5 text-white" />}
          {!isEmptyState && (
            <p
              className={`font-medium text-white whitespace-nowrap ${metric ? "text-[10px]" : "text-[11px]"}`}
            >
              {t("editor.addDesigns")}
            </p>
          )}
      ...
      {isEmptyState && (
        <p className="text-center mt-4 text-sm font-medium text-gray-600">
          {t("upload.poweredBy")}{" "}
          <span className="text-cyan-600 font-semibold">ANYNEST APP</span>
        </p>
      )}
    </div>
  );
