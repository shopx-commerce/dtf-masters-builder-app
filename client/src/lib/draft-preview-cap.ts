 ...
 * draws, capped at `MAX_STORED_IMAGE_DIMENSION` on upload, and `exportBlob`,
 * the full-resolution print source. A draft persists one blob per design, and
 * that blob is deliberately the uncapped one so print resolution survives
 * recovery — which leaves restore holding a decoded preview far larger than the
 * session that saved it (a 4096 px prepared preview is ~67 MB of RGBA against
 * the ~16 MB the editor budgeted, per design).
 *
 * So restore mirrors the upload path instead: `image` is capped the same way,
 * and the blob it was decoded from becomes the print source, so the export path
