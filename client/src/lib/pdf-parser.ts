...
import { assertVectorFileWithinLimit } from './image-budget';
 ...
 * substituted font that looks nothing like their file. The `pdfjsAssets`
 * plugin in `vite.config.ts` copies these out of `pdfjs-dist` into the public
 * directory, keyed to the installed version so a dependency bump cannot leave
 * stale CMaps behind.
 *
...
export async function parsePDF(file: File): Promise<ParsedPDFData> {
  // Local re-check of the size ceiling. `file.arrayBuffer()` below reads the
  // whole document into memory, and this function is exported, so a caller that
  // does not sit behind `rejectOversizedVector` would happily read 400 MB.
