 ...
 * regardless of ownership — see `purgeEditorDraftStorage`. Falls back to a
 * one-shot channel because the caller is a tab whose editor has already been
 * torn down by the error boundary, so `acquireDraftOwnership`'s channel may
 ...
 * run on mount (the expiry purge, the recovery offer) await this so they cannot
 * act on another tab's live record during the election window.
 */
...
/** Test seam: the harness needs to observe which tab won. */
export function draftOwnershipTabId(): string {
  return tabId;
}

/**
 * Dev-only probes, on the same footing as `window.__stressUpload`. Tree-shaken
 * out of production builds.
 *
 * `__draftOwnershipKill` is the one that earns its keep: an owner that is killed
 * rather than closed — OOM on mobile, a renderer crash, a force-quit — is the
 * failure mode that a durable lock turns into a permanent outage, and it cannot
 * be reproduced from a driver script any other way. It stops the heartbeat
 * without broadcasting a release, which is precisely what every other tab sees
 * when a tab dies, so the lease timeout can be exercised for real.
 */
if (import.meta.env?.DEV && typeof window !== "undefined") {
  Object.assign(window as unknown as Record<string, unknown>, {
    __draftOwnership: () => ({ ...state, tabId }),
    __draftOwnershipKill: () => {
      stopHeartbeat();
      ...
      return "heartbeat stopped without releasing";
    },
  });
}
