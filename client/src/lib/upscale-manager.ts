...
import { MAX_INLINE_DECODE_MEGAPIXELS, fitWithinMegapixels } from "./image-budget";
import { detectUpscaleSupport } from "./upscale-support";
import { vectorExportMaxEdge } from "./vector-raster-limits";
 ...
 * (`MAX_INLINE_DECODE_MEGAPIXELS`): an upscale that blew past it would put a
 * design into the canvas the export path cannot hold, turning "increase
 * quality" into a failed checkout.
 */
export function resolveUpscaleScale(width: number, height: number, requested: number): number {
  ...
  for (const scale of [...UPSCALE_FACTORS].reverse()) {
    ...
    const fits = fitWithinMegapixels(width * scale, height * scale, MAX_INLINE_DECODE_MEGAPIXELS, maxEdge);
