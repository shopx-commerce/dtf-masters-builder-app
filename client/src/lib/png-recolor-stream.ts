...
export type StreamAnalysis =
  | { eligible: true; sourceColor: RgbColor; width: number; height: number }
  | { eligible: false; reason: ColorChangeReason; width?: number; height?: number };
...
/** Mutable single-ink state: -1 until the first visible pixel names the ink. */
interface InkState { r: number; g: number; b: number }

const ROW_OK = 0;
const ROW_CONFLICT = 1;
...
/**
 * Builds the per-row pixel walk for this colour type.
 *
 * One specialised loop per format rather than one loop with a switch inside it:
 * this runs once per pixel of a print-resolution image, so a branch here is
 * tens of millions of branches per recolour.
 */
function makeRowProcessor(
  meta: PngMeta,
  crop: SourceCrop,
  target: RgbColor | null,
  ink: InkState,
): RowProcessor {
  const x0 = crop.x;
  ...
  if (meta.colorType === 6) {
    return (row, out) => {
      for (let x = x0, o = 0; x < x1; x++, o += 4) {
        ...
        const a = row[s + 3];
        if (a !== 0) {
          const r = row[s], g = row[s + 1], b = row[s + 2];
          if (ink.r < 0) { ink.r = r; ink.g = g; ink.b = b; }
          else if (r !== ink.r || g !== ink.g || b !== ink.b) return ROW_CONFLICT;
          if (out) { out[o] = tr; out[o + 1] = tg; out[o + 2] = tb; out[o + 3] = a; }
        } else if (out) {
          // Invisible pixels keep the RGB the artwork stored under them, which
          // is what makes this a recolour of the ink rather than a rewrite of
          // the file.
  ...
  if (meta.colorType === 4) {
    return (row, out) => {
      for (let x = x0, o = 0; x < x1; x++, o += 4) {
        ...
        const a = row[s + 1];
        if (a !== 0) {
          if (ink.r < 0) { ink.r = grey; ink.g = grey; ink.b = grey; }
          else if (grey !== ink.r || grey !== ink.g || grey !== ink.b) return ROW_CONFLICT;
          if (out) { out[o] = tr; out[o + 1] = tg; out[o + 2] = tb; out[o + 3] = a; }
        } else if (out) {
  ...
  // Expanded to a full 256 entries up front, with a validity flag per index:
  ...
  const entries = Math.floor(sourcePalette.length / 3);
  const paletteRgb = new Uint8Array(256 * 3);
  const paletteAlpha = new Uint8Array(256);
  const defined = new Uint8Array(256);
  for (let i = 0; i < Math.min(256, entries); i++) {
    paletteRgb[i * 3] = sourcePalette[i * 3];
    paletteRgb[i * 3 + 1] = sourcePalette[i * 3 + 1];
    paletteRgb[i * 3 + 2] = sourcePalette[i * 3 + 2];
    paletteAlpha[i] = i < sourceTransparency.length ? sourceTransparency[i] : 255;
    defined[i] = 1;
  }
  return (row, out) => {
    for (let x = x0, o = 0; x < x1; x++, o += 4) {
      const index = row[x];
      if (!defined[index]) return ROW_INVALID;
      const p = index * 3;
      const a = paletteAlpha[index];
      const r = paletteRgb[p], g = paletteRgb[p + 1], b = paletteRgb[p + 2];
      if (a !== 0) {
        if (ink.r < 0) { ink.r = r; ink.g = g; ink.b = b; }
        else if (r !== ink.r || g !== ink.g || b !== ink.b) return ROW_CONFLICT;
        if (out) { out[o] = tr; out[o + 1] = tg; out[o + 2] = tb; out[o + 3] = a; }
      } else if (out) {
        out[o] = r; out[o + 1] = g; out[o + 2] = b; out[o + 3] = 0;
      }
    }
    return ROW_OK;
  };
...
type StreamOutcome =
  | { ok: true; sourceColor: RgbColor; width: number; height: number; blob: Blob | null }
  | { ok: false; reason: ColorChangeReason; width?: number; height?: number }
  /** This file needs the whole-image decoder — interlaced, or otherwise not a
   *  format that can be walked row by row. */
  | null;

/**
 * Walks the file once, proving the single-ink rule and (when `target` is given)
 * writing the recoloured copy as it goes.
 *
 * Recolour verifies rather than trusting a previous analysis: nothing is
 * committed until the last row, so discovering a second ink halfway through
 * costs the work done so far and nothing else.
 */
    ...
    for (;;) {
      ...
      if (type === "IHDR") {
        ...
        if (!rowsSettled) {
          ...
          sink = target ? new RecoloredPngSink(crop.width, crop.height, meta.physical) : null;
...
async function consumeRows(
  ...
  const ink: InkState = { r: -1, g: -1, b: -1 };
  const processRow = makeRowProcessor(meta, crop, target, ink);
  ...
  try {
    ...
    return await finish({
      ok: true,
      sourceColor: { r: ink.r, g: ink.g, b: ink.b },
      width: crop.width,
      height: crop.height,
      blob,
    });
...
/** Proves (or disproves) the single-ink rule without holding the image. */
export async function streamAnalyzePng(
  blob: Blob,
  crop?: SourceCrop,
  options?: StreamOptions,
): Promise<StreamAnalysis | null> {
  const outcome = await runStream(blob, null, crop, options);
  if (outcome === null) return null;
  if (!outcome.ok) return { eligible: false, reason: outcome.reason, width: outcome.width, height: outcome.height };
  return {
    eligible: true,
...
/** Rewrites every visible pixel to `target`, preserving alpha byte for byte. */
export async function streamRecolorPng(
  ...
  const outcome = await runStream(blob, target, crop, options);
  if (outcome === null) return null;
