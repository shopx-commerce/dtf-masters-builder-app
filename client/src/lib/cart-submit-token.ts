 ...
 * its shell relay.
 *
 * NOTE: this module is intentionally standalone for now. Wiring it into the
 * add-to-cart flow (minting an id when a submit goes out, checking it when a
 * status comes back, discarding it on a failed send) is deliberately deferred
 * to a separate effort so the cart flow itself is not touched here.
 *
