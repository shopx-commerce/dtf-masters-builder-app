 ...
 *
 * The image editor's design-state hook owns a `useRef<Map<string, string>>`
 * where the key is the source image's blob URL (or design id) and the value
 * is either a `blob:` URL from the thumbnail worker or a
 * `data:image/png;base64,…` URL from the main-thread fallback. Without a cap
 * the map grows for the lifetime of the tab — every uploaded design leaves a
 * permanent entry behind, and every `blob:` URL keeps its decoded thumbnail
 * pinned in browser memory.
 *
