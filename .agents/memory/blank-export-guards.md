...
**Diagnosing "the file didn't update":** production assets serve from stable keys with `cache-control: public, max-age=300` (Cloudflare itself reports DYNAMIC, i.e. no edge caching — the stale layer is the *browser*). A shop owner who re-checks the same URL right after an update sees their old download, not the new bytes. Always verify server truth with a cache-busted fetch (`?cb=<ts>`) + alpha scan before concluding an update failed; the state JSON's `production.url?v=N` and the file's `last-modified` tell you whether/when a re-upload actually happened.
