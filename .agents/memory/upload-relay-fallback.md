...
- The legacy `/api/upload-design` form-POST route gets no fallback: shells old enough to hand out that URL predate the relay handlers, so the retry would only add the timeout stall.
